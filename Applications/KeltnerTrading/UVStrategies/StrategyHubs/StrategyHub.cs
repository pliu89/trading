...
namespace UV.Strategies.StrategyHubs
{
    ...
    public class StrategyHub : Hub, IService, IStringifiable, IEngineHub
    {
        ...
        public bool SubscribeToMarketInstruments(List<InstrumentName> instrumentList, Strategy strategy)
        {
            ...
            if (m_Market == null)
                return false;
            Log.BeginEntry(LogLevel.Minor, "SubscribeToMarketInstruments: Request to subscribe to {0} instruments from strategy {1}.", instrumentList.Count, strategy.Name);
            ...
            if (nInstrSubscriptionsRequested > 0)
            {
                ...
                foreach (InstrumentName instrumentName in m_InstrumentSubscriptionsOutstanding)
        ...
        private void ProcessRequestLoadStrategies(RequestEventArg<RequestCode> requestArg)
        {
            ...
            else if (m_StrategyGroupIds != null && m_StrategyGroupIds.Count > 0)
            {
                ...
                if (StrategyMaker.TryCreateFromDatabase(m_StrategyGroupIds, this.Log, m_DBReaderWriter, out newStrategies) && newStrategies.Count > 0)
                {
                    Log.NewEntry(LogLevel.Error, "ProcessRequestLoadStrategies: Created {0} strategies. Initializing.",newStrategies.Count);
                    InitializeStrategies(newStrategies);
                }
                else
        ...
        private void InitializeStrategies(List<Strategy> newStrategies)
        {
            ...
            foreach (Strategy strategy in newStrategies)
            {
                ...
                StrategyEngines.ZGraphEngine zGraphEng = null;                          // this is engine we want to include
                foreach (IEngine iEng in engineList)
                {
                    if (iEng is StrategyEngines.ZGraphEngine)
                        zGraphEng = (StrategyEngines.ZGraphEngine)iEng;
                    // search for others here.
                }
                if (zGraphEng == null)
                {
                   zGraphEng = new StrategyEngines.ZGraphEngine();
            ...
            foreach (Strategy strategy in newStrategies)
            {
                ...
                {
                    Log.NewEntry(LogLevel.Minor, "InitializeStrategies: Strategy #{0} {1} not ready for complete setup.",strategy.ID, strategy.Name);
        ...
        */
        ...
        protected void ProcessCompletedQuery(Queries.QueryBase eventArg)
        {
            ...
            if (m_PendingQueryCallback.TryGetValue(eventArg.QueryID, out h))
            {
                ...
                if (strategy != null)
                    Log.NewEntry(LogLevel.Minor, "ProcessDBReaderWriter: Processing callback for strategy {0}.  {1} queries remaining.",strategy.Name,m_PendingQueryCallback.Count);
        ...
        /// during their SetupBegin phase, will inform the me when they want
