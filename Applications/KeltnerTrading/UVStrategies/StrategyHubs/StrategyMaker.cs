...
namespace UV.Strategies.StrategyHubs
{
    ...
    public static class StrategyMaker
    {
        ...
        /// <returns></returns>
        public static bool TryCreateFromDatabase(List<int> groupId, LogHub aLog, DatabaseReaderWriter dataBaseReader, out List<Strategy> strategyList)
        {
            strategyList = new List<Strategy>();                                    // place to put new strategies
            ...
            try
            {
                ...
                // Request strategies.
                //
                StrategiesQuery strategyQuery = new StrategiesQuery(groupId);
                if (!dataBaseReader.SubmitSync(strategyQuery))
                {
                    aLog.NewEntry(LogLevel.Error, "TryCreateFromDatabase: groupId {0} query failed, cannot create strategy",groupId);
                    return false;
                }
                ...
                {
                    aLog.NewEntry(LogLevel.Error, "TryCreateFromDatabase: groupId {0} query failed try to find engines for {1} strategies, cannot create strategy",groupId, strategyIdList.Count);
                    return false;
                }
