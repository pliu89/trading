...
namespace Ambre.PositionViewer.FrontEnds
{
    ...
    public partial class FillHubPage : TabPage
    {
        ...
        public FillHubPage(MarketTTAPI marketHub)
        {
            InitializeComponent();
        ...
        public void AddHub(FillHub newFillHub)
        {
            ...
            m_MarketHub = m_FillHub.MarketHub;
            if (Log == null)
        ...
        public void DeleteHub()
        {
            ...
            // TODO:    1) Send request to fillhub to flatten all positions, ideally, this puts in UserAdjustment fills to zero out positions.
            //          2) Send request to fillHub to drop a snapshot, archive etc.
            //          3) Ask fillHub to shut down.
            m_FillHub.Request(new Misty.Lib.OrderHubs.OrderHubRequest(Misty.Lib.OrderHubs.OrderHubRequest.RequestType.RequestCompleteBookReset));
            ...
            Shutdown();                                             // disconnect from all the form and external events, stop updating.
        ...
        //
        public void UpdateInstrumentInfos()
        {
            if (m_IsShuttingDown)
                return;
        ...
        private void FindInstrumentInfo()
        {
            ...
            foreach (string name in m_InstrumentInfos.Keys)
            {
                ...
                if (!info.IsFoundInMarket)
                {
                    IFillBook book;

                    if (instrumentName.IsCashProduct)
                    {
                        // Found details for this cash instrument, update things.
                        if (!m_FillHub.TryEnterReadBook(instrumentName, out book))
                            Log.NewEntry(LogLevel.Warning, "No cash book added to the fill hub, and the cash instrument is {0}.", instrumentName);
                        else
                        {
                            info.IsFoundInMarket = true;

                            // Show the cash instrument currency name and currency rate.
                            // The currency name and currency rate sources from the delete click on that instrument and these values are loaded from the fill book for that deleted instrument.
                            CashBook cashBook = (CashBook)book;
                            m_InstrumentInfos[instrumentName.FullName].CurrencyCode = cashBook.CurrencyName;
                            m_InstrumentInfos[instrumentName.FullName].CurrencyRate = cashBook.CurrencyRate;
                            m_FillHub.ExitReadBook(instrumentName);

                            // Upadte the cash instrument on GUI again.
                            FillHub.PositionBookChangedEventArgs e = new FillHub.PositionBookChangedEventArgs();
                            e.Instrument = instrumentName;
                            FillHub_PositionBookCreated(this, e);
                        }
                    }
                    // If the market close for some instruments like Tokyo products for example, instrument detail will be null and we can not subscribe to the price for that instrument.
                    else
                    {
                        if (m_MarketHub.TryLookupInstrumentDetails(instrumentName, out details))
                        {   
                            ...
                            // The currency rate for the instrument may become useless in the future.
                            double x = details.Currency.GetConversionRate(TradingTechnologies.TTAPI.Currency.PrimaryCurrency);
                            string goodCurrencyName = details.Currency.Code;
                            if (double.IsNaN(x) || double.IsInfinity(x))
                            {
                                // If the given currency is invalid like NaN or infinity, assign zero to the contract.
                                if (m_FillHub.TryEnterReadBook(instrumentName, out book))
                                {
                                    // Change the currency rate in the fill book if the TT value is good.
                                    BookLifo fillBook = (BookLifo)book;
                                    if (!string.IsNullOrEmpty(goodCurrencyName))
                                        fillBook.CurrencyName = goodCurrencyName;
                                    else
                                        goodCurrencyName = fillBook.CurrencyName;

                                    fillBook.CurrencyRate = 1;
                                    m_FillHub.ExitReadBook(instrumentName);
                                }
                            ...
                            {
                                // We have the currency rate value in book, loaded from the xml file as an attribute.
                                if (m_FillHub.TryEnterReadBook(instrumentName, out book))
                                {
                                    // Change the currency rate in the fill book if the TT value is good.
                                    BookLifo fillBook = (BookLifo)book;
                            ...
                            m_InstrumentInfos[instrumentName.FullName].CurrencyCode = goodCurrencyName;     // If the instrument detail exists, the currency code seems to be valid in any case.
                        ...
                        {
                            // Only show the currency information. No instrument details is needed in this case.
                            if (m_FillHub.TryEnterReadBook(instrumentName, out book))
                            {
                                // Change the currency rate in the fill book if the TT value is good.
                                BookLifo fillBook = (BookLifo)book;
                                m_InstrumentInfos[instrumentName.FullName].CurrencyCode = fillBook.CurrencyName;
                                m_FillHub.ExitReadBook(instrumentName);
                                ...
                                Log.NewEntry(LogLevel.Warning, "No fill book added to the fill hub, and the instrument is {0}.", instrumentName);
                        }
                    }
                }
            }// next instrument
            ...
            foreach (string instrName in m_InstrumentInfos.Keys)
            {
                ...
                if (info.IsPriceChanged)
                {
                    ...
                    if (m_FillHub.TryEnterReadBook(instrumentName, out positionBook))
                    {
                        ...
                        info.UnrealPnL = positionBook.UnrealizedDollarGains(info.Price);
                        info.RealPnL = positionBook.RealizedDollarGains;                                        // overkill, but useful at start up.
                        m_FillHub.ExitReadBook(instrumentName);
        ...
        private void ActiveGridUpdatePosition(FillHub.PositionBookChangedEventArgs eventArgs)
        {
            ...
            if (m_FillHub.TryEnterReadBook(instrumentName, out positionBook))
            {
                ...
                info.UnrealPnL = Math.Round(positionBook.UnrealizedDollarGains(), 2);                            // only this can change outside of position changing.
                ...
                && this.activeGrid1.Items.ContainsKey(string.Format("{0}{1}", GroupRowNamePrefix, productGroupKey)))
            {
                ...
                foreach (string instrName in m_ProductGroups[productGroupKey])
                    if (m_InstrumentInfos.TryGetValue(instrName, out info))
                    {
                        ...
                        //sumStartRealPnL += info.StartingRealPnL * info.CurrencyRate;
                        //sumUnRealPnL += info.UnrealPnL * info.CurrencyRate;
                        //sumRealPnL += info.RealPnL * info.CurrencyRate;
                        //sumPnL += info.TotalPnL * info.CurrencyRate;
                    }
