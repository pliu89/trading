...
namespace Ambre.TTServices.Fills
{
    ...
    public class FillListener : IDisposable, IStringifiable
    {
        ...
        public string m_TradeFilterArg = string.Empty;              // The key associated with the filter. 
        public InstrumentKey m_LastInstrumentKey;                   // Last filter instrument key.
        ...
        #region Property
        public TradeSubscriptionFilter TradeSubscriptionFilter
        {
            get { return m_TradeFilter; }
            set { m_TradeFilter = value; }
        }
        #endregion
        ...
        public void FilterByInstrument(InstrumentKey instrumentKey)
        {
            ...
            this.m_TradeFilter = filter;
            m_LastInstrumentKey = instrumentKey;
        ...
        private void TT_OrderFilled(object sender, TradingTechnologies.TTAPI.OrderFilledEventArgs eventArgs)
        {
            ...
            OnFilled(CreateFillEventArg(FillType.New, eventArgs.Fill));
        }
        //
        /// <summary>
        ...
        {
            OnFilled(CreateFillEventArg(FillType.Adjustment, eventArgs.Fill));
        }
        ...
        public string GetAttributes()
        {
            ...
            if (m_TradeFilter != null)
            {
                ...
                {
                    Log.NewEntry(LogLevel.Warning, "Unknown filter happens and the trade filter member value is {0}", m_TradeFilter);
                    if (!string.IsNullOrEmpty(m_TradeFilterArg))
                    {
                        if (m_FilterType == FilterType.Account)
                            msg.AppendFormat(" FilterAccount={0}", m_TradeFilterArg);
                        if (m_FilterType == FilterType.Instrument)
                            msg.AppendFormat(" FilterInstrumentKey={0}", m_TradeFilterArg);
                    }
                }
            }
            ...
            {
                Log.NewEntry(LogLevel.Warning, "null filter happens");
                if (!string.IsNullOrEmpty(m_TradeFilterArg))
                {
                    if (m_FilterType == FilterType.Account)
                        msg.AppendFormat(" FilterAccount={0}", m_TradeFilterArg);
