...
namespace Ambre.TTServices.Talker
{
    ...
    public class TalkerHub : Hub , IService , IStringifiable
    {
        ...
        public TalkerHub(bool isLogViewerVisible, int ambreXLPort=6012)
            : base("Talker", Misty.Lib.Application.AppInfo.GetInstance().LogPath, isLogViewerVisible, LogLevel.ShowAllMessages)
        {
            #if (!DEBUG)
        ...
        {
            //Misty.Lib.Application.AppServices sevices = Misty.Lib.Application.AppServices.GetInstance();
        ...
        */ 
        ...
        {
            // Update all markets
            Misty.Lib.BookHubs.Book mktBook = null ;
                ...
                switch (topic.Type)
                {
                    ...
                    case SubscriptionType.StartingVolume:
                        if (m_FillHub.TryEnterReadBook(topic.Instrument, out book))
                        {
                            ...
                            topic.SetValue("");
                        break;
                    case SubscriptionType.CurrencyName:
                        if (m_FillHub.TryEnterReadBook(topic.Instrument, out book))
                        {

                            topic.SetValue(book.CurrencyName.ToString());
                            m_FillHub.ExitReadBook(topic.Instrument);
                        }
                        else
                            topic.SetValue("");
