...
namespace UV.Lib.IO.Xml
{
    ...
    public static class Stringifiable
    {
        ...
        public static bool TryGetType(string typeFullName, out Type type)
        {
            ...
            // Try to get the type from our cache, or current assemblies.
            lock (m_TypeCache)
            {
        ...
        //
        //
        public static bool TryCreate(string strigifiedObjects, out List<IStringifiable> objects, bool createNodes=false)
        {
            objects = null;
            bool isSuccess = false;
            try
            {
                objects = Create(strigifiedObjects,createNodes);
                isSuccess = (objects != null);
            }
            catch (Exception)
            {
                isSuccess = false;
        ...
        /// <returns></returns>
        public static string Stringify(IStringifiable target, Dictionary<Type, string[]> overrideFunctionTable = null, bool useNiceFormat = true)
        {
            return Stringify(0, target, overrideFunctionTable, useNiceFormat);
        }
        // 
        // 
        //
        //
        //
        private static string Stringify(int level, IStringifiable target, Dictionary<Type, string[]> overrideFunctionTable, bool useNiceFormat)
        {
            if (target is IStringifiable)
            {
                ...
                string attributes = string.Empty;                               // default attributes is empty, and 
                ...
                {   // User has told us that this object IStringifiable functions should been overridden by other functions.
                    string[] funcNames;
                    ...
                    {                        
                        ...
                        // Object type name override.  Here, user can provide a name directly or a new function.
                        if (string.IsNullOrEmpty(funcNames[0]))
                            targetTypeFullName = targetType.FullName;           // if empty, use objects true name
                        else
                        {   
                            MethodInfo mInfo = targetType.GetMethod(funcNames[0]);// See if override function exists, and is public
                            if (mInfo.IsPublic && mInfo.GetParameters().Length == 0 && mInfo.ReturnType == typeof(string))
                                targetTypeFullName = (string)mInfo.Invoke(target, null);    // if so, use the function.
                            else
                                targetTypeFullName = funcNames[0];              // otherwise override with provided name.
                        }
                        // Attribute
                ...
                {   // This is the default case to use IStringifiable functions.
                    // When no override dictionary entry exists for this type.
                ...
                msg.AppendFormat("{0}<{1}",space, targetTypeFullName);     // Create starting tag
                if (!String.IsNullOrEmpty(attributes))                      // It may have no attributes...
                    msg.AppendFormat(" {0}", attributes);                   // or add them after first tag.
                // Now, add sub elements.
                if (elements != null && elements.Count > 0)
                {
                    msg.Append(">");                                        // Close the starting tag.
                    foreach (IStringifiable element in elements)            // add sub elements
                        msg.AppendFormat("{0}", Stringify(level + 1, element, overrideFunctionTable, useNiceFormat));
