...
namespace UV.Strategies.ExecutionEngines.OrderEngines
{
    ...
    /// </summary>
    public class QuoterLeg : Engine, IStringifiable
    {
        ...
        // Current Fills
        private List<FillBook> m_FillBooks = new List<FillBook>();
        #endregion// members
        ...
        private bool UpdateLeanablePrices(out bool isLeanableStatusChange)
        {
            ...
            for (int side = 0; side < 2; ++side)
            {
                if (m_Market.BestDepthUpdated[side] > m_LeanableDepth[side])    // the last update on this side didn't affect our pricing
                    continue;
        ...
        /// </summary>
        /// <param name="isLeanablePriceChanged"></param>
        ...
        void IStringifiable.AddSubElement(IStringifiable subElement)
        {
            ...
            else if (type == typeof(Hedger))
            {
                ...
                m_Hedger.m_QuoterLeg = this;            // give pointer to ourself
            }
