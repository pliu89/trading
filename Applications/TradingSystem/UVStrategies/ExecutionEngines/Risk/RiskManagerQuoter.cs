...
namespace UV.Strategies.ExecutionEngines.Risk
{
    ...
    /// </summary>
    public class RiskManagerQuoter :  RiskManager, IStringifiable
    {
        #region Members
        // *****************************************************************
        // ****                     Members                             ****
        ...
        public ExecutionEngines.OrderEngines.Quoter m_Quoter;
        #endregion// members
        ...
        public override void SetupBegin(Lib.Engines.IEngineHub myEngineHub, Lib.Engines.IEngineContainer engineContainer)
        {
            ...
            { // Find order Engine
                if (iEng is Quoter)
                    this.m_Quoter = (Quoter)iEng;                                             // find our order engine
        ...
        {
            base.SetupComplete();
            foreach (ExecutionEngines.OrderEngines.QuoterLeg quoterLeg in m_Quoter.m_QuoterLegs)
            ...
            m_Quoter.m_IExecutionListener.GetAllOrderBooks(ref orderBookList);
            foreach (OrderBook orderBook in orderBookList)
            {
                orderBook.OrderSubmitted += new EventHandler(OrderBook_OrderSubmitted); // subscribe to all order books created.
            }
        }
        ...
        public override int FatFingerTicks
        {
            ...
            set
            {
                ...
                { //we have a new value
                    ...
                    m_FatFingerPriceTolerance = m_Quoter.QuoteTickSize * value;
                }
            }
        }
        ...
        public override void Start()
        {
            ...
            m_Quoter.IsRiskCheckPassed = true;                          // let our order engine begin to send orders.
        }
        //
        ...
        //***********************************************************
        /// <summary>
        /// Aggregate total PnL across all legs of the spread.
        /// </summary>
        /// 
        //
        /// <summary>
        ...
        private bool UpdateAndCheckSpreaderPnL()
        {
            m_PnL = 0; // reset PnL to 0
            if (m_Quoter.m_IsLegSetupCompleted)
            {
                for (int i = 0; i < m_Quoter.m_LegFillBooks.Length; i++)
                {

                    double midPrice = (m_Quoter.m_QuoterLegs[i].m_Market.Price[UV.Lib.Utilities.QTMath.BidSide][0] +
                        m_Quoter.m_QuoterLegs[i].m_Market.Price[UV.Lib.Utilities.QTMath.AskSide][0]) / 2;

                    m_PnL += (m_Quoter.m_LegFillBooks[i].m_RealizedGain +              // (realized + unrealized) = PnL
                        m_Quoter.m_LegFillBooks[i].UnrealizedDollarGains(midPrice));
                }
        ...
        public bool CheckLegPositions()
        {
            ...
            {// iterate through all fill books and make sure no leg is grossly out of whack
                long MaxSensicalLongQtyForLeg = (long)((Math.Abs(m_Quoter.m_LegRatios[i] * m_Quoter.m_TotalDesiredQty[UV.Lib.Utilities.QTMath.MktSignToMktSide(m_Quoter.m_LegRatios[i])])) +  // Qty * Ratio + Max Possibly Overfilled On
                    (m_Quoter.m_DripQty * (m_Quoter.m_LegRatios.Length - 1) * Math.Abs(m_Quoter.m_LegRatios[i]))); // if we are quoting all the legs and we get filled across everything this could happen

                long MaxSensicalShortQtyForLeg = (long)(((Math.Abs(m_Quoter.m_LegRatios[i] * m_Quoter.m_TotalDesiredQty[UV.Lib.Utilities.QTMath.MktSignToMktSide(m_Quoter.m_LegRatios[i] * -1)])) +  // Qty * Ratio + Max Possibly Overfilled On
                        (m_Quoter.m_DripQty * (m_Quoter.m_LegRatios.Length - 1) * Math.Abs(m_Quoter.m_LegRatios[i]))) * -1); // if we are quoting all the legs and we get filled across everything this could happen

                long ExpectedQtyForLeg = (long)(Math.Abs((m_Quoter.FillBook.m_NetPosition * m_Quoter.m_LegRatios[i])) +        // Our Spread Positon * LegRatio +
                    Math.Abs(m_Quoter.m_DripQty * m_Quoter.m_LegRatios[i] * m_Quoter.m_MaxAllowableHangs * 2) - 1);                                 // DripQty * LegRatio * # of PossibleHangs + quanity if we are partialled 

                if (m_Quoter.m_LegFillBooks[i].m_NetPosition > MaxSensicalLongQtyForLeg || m_Quoter.m_LegFillBooks[i].m_NetPosition < MaxSensicalShortQtyForLeg)
                { // our position exceeds the possible max 
                    m_Log.NewEntry(LogLevel.Error, "Our Position in {0} = {1} Which is outside our Max expected Long Qty of {2} or Short Qty of {3}",
                        m_Quoter.m_QuoterLegs[i].m_PriceLeg.InstrumentName, m_Quoter.m_LegFillBooks[i].m_NetPosition, MaxSensicalLongQtyForLeg, MaxSensicalShortQtyForLeg);
                    isPositionLogical = false;  // set our bool to true stating risk should be triggered
                    break;
                }
                else if (Math.Abs(m_Quoter.m_LegFillBooks[i].m_NetPosition) > (ExpectedQtyForLeg))
                { // our position exceeds what we would logically expect.
                    m_Log.NewEntry(LogLevel.Error, "Our Position in {0} = {1} Which is greater than our expected Qty of {2} based on our sytnthetic position of {3}",
                       m_Quoter.m_QuoterLegs[i].m_PriceLeg.InstrumentName, m_Quoter.m_LegFillBooks[i].m_NetPosition, ExpectedQtyForLeg, m_Quoter.FillBook.m_NetPosition);
                    isPositionLogical = false;  // set our bool to true stating risk should be triggered
                    break;
                }
            }
        ...
        protected override bool CheckWorkingOrderCounts()
        {
            ...
            long maxTotalWorkQuoteQty = m_MaxPossibleWorkingQuoteQtyPerLot * m_Quoter.m_DripQty; // this is assuming we are quoting both sides and all legs
            bool isTooManyWorkingOrders = false;

            for (int i = 0; i < m_Quoter.m_OrderBooks.Count; i++)
                totalQuoteCount += m_Quoter.m_OrderBooks[i].m_TotalWorkingOrderQty;     // sum all working quote orders in market for book 
        ...
        {
            base.FlagRiskEvents();
            m_Quoter.OnQuoterStateChange();                 // push to quoter and then to legs.
        }
        ...
        {
            if (UpdateAndCheckSpreaderPnL() | CheckQuoteLimits())    // max loss pnl tripped or max number of quotes per second exceeded
                FlagRiskEvents();
            ...
            base.BroadcastParameter((IEngineHub)m_Quoter.m_Hub, m_Quoter.m_ExecutionContainer, 4);
            base.BroadcastParameter((IEngineHub)m_Quoter.m_Hub, m_Quoter.m_ExecutionContainer, 5);
        }
        #endregion
    }
}
