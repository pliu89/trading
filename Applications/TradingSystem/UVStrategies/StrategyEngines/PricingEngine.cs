...
namespace UV.Strategies.StrategyEngines
{
    ...
    public class PricingEngine : Engine, IStringifiable, IPricingEngine
    {
        ...
        protected int CustomEventPaddingSeconds = -1;                // -1 means we don't care about custom events           
        protected bool IsIncludeCustomEventsAsNormalEvents = true;   // forces all custom events to be treated with the same blocking period as normal events
        ...
        // *****************************************
        /// <summary>
        /// This method performs some basic functions useful for more PricingEngines, such as collecting pointers
        /// to the parent Strategy, and the StrategyHub Log.  It also locates the OrderEngine (to which it sends quotes), 
        /// and requests EconomicData (upcoming economic number announcements etc.) for each of its legs.
        /// </summary>
        ...
        /// pointers to them.
        /// It also automatically requests MarketChange subscriptions from its StrategyHub for each
        /// instrument listed as one of its PriceLegs.
        /// </summary>
        /// <param name="myEngineHub"></param>
        /// <param name="engineContainer"></param>
        public override void SetupBegin(IEngineHub myEngineHub, IEngineContainer engineContainer)
        {
            ...
            //
            // Subscribe to PricingLeg instruments.
            //
        ...
        public void SetUpAlarms()
        {
            ...
            // Alarms for Events.
            //
            if (m_EconomicDataDateTime != null && m_EconomicDataUnique != null && (EventPaddingSeconds > -1 || CustomEventPaddingSeconds > -1))
            {
                ...
                { //for each unique time 
                    ...
                    Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} Economic event block @ {2} for events:", ParentStrategy.Name, m_EngineName, stopAlertTime);
                    ...
                    while (m_EconomicDataDateTime.Count > nextEventPtr)                                 // loop thru following events, to check for overlapping blocking periods.
                    {
                        ...
                        {   // if the next alarm (to stop trading) is within 5 seconds after current start-trading time.
                            ...
                            Log.NewEntry(LogLevel.Major, "SetUpAlarms: Overlapping blocking periods found. Combining!", ParentStrategy.Name, m_EngineName);
                        }
                        else
                    ...
                    { // the actual event already happened
                        ...
                        { // the blocking hasn't passed yet however, so put us in a blocking state!
                            ...
                            Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} Starting strategy in blocking state!", ParentStrategy.Name, m_EngineName);
                        }
                    }
                }
                ...
                Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} Created alarms for {2} economic events", ParentStrategy.Name, m_EngineName, m_EconomicDataUnique.Count);
            }
            else
                Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} Failed to set up any economic events", ParentStrategy.Name, m_EngineName);
            ...
            { // the model has specified a start time for itself
                ...
                { // We have a trading start time and it is later than right now.
                    ...
                    Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} TradingStartTimer @ {2}", ParentStrategy.Name, m_EngineName, startTradingDateTime);
                }
                else
                { // trading start time has already passed for today, lets just set a time in case we want to start trading at this time tomorrow
                    ...
                    Alarm.Set(startTradingDateTime, ParentStrategy.StrategyHub.HubEventEnqueue, startEventArgs);
                    Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} TradingStartTimer is {2}", ParentStrategy.Name, m_EngineName, startTradingDateTime);
                }
            }
            ...
            if (TradingEndTime != null)
            {
                ...
                { // we have and end time that is in the future.
                    ...
                    Log.NewEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} TradingEndTimer @ {2}", ParentStrategy.Name, m_EngineName, stopTradingDateTime);
                }
                else
            ...
            // Debug - add a custom event 5 minutes from now
            //TimeSpan test = new TimeSpan(0,4,0);
            //TimeSpan newTimeSpan = DateTime.Now.TimeOfDay.Add(test);
            //CustomEvents.Add(newTimeSpan);
            //
            if (CustomEvents.Count != 0)
            { // we have times to avoid
                Log.BeginEntry(LogLevel.Major, "SetUpAlarms: {0}:{1} Found {2} custom events to include.", ParentStrategy.Name, m_EngineName, CustomEvents.Count);
                
                int customPadSeconds = CustomEventPaddingSeconds;
                if (IsIncludeCustomEventsAsNormalEvents)                                                    // if the user wants custom events to be treated differently than normal!
                    customPadSeconds = System.Math.Max(EventPaddingSeconds, CustomEventPaddingSeconds);     // just for safety, take the max
                Log.AppendEntry(" Padding={0} seconds.", customPadSeconds);

                Log.AppendEntry(" Event times:");
                foreach (TimeSpan time in CustomEvents)
                {
                    DateTime eventTime = DateTime.Today.Add(time);
                    Log.AppendEntry(" {0}", eventTime.ToShortTimeString());

                    //
                    // Set Up Start of Custom Events - Padding by an extra day should always work for our current needs.
                    //
                    DateTime eventBlockStart = eventTime.AddSeconds(-customPadSeconds);
                    EngineEventArgs startEventArgs = eventArgToCopy.Copy();
                    ...
                    //
                    DateTime eventBlockingEnd = eventTime.AddSeconds(customPadSeconds);
                    EngineEventArgs stopEventArgs = eventArgToCopy.Copy();
        ...
        //
        // *****************************************************
        // ****             IsAllPricingLegsGood           ****
        // *****************************************************
        /// <summary>
        /// Caller would like to check that all legs for this pricing engine are in a "Good" state
        /// </summary>
        /// <param name="marketBook"></param>
        /// <returns></returns>
        protected bool IsAllPricingLegsGood(Book marketBook)
        {
            bool IsAllLegsGood = true;
            foreach(PriceLeg leg in m_Legs)
            { 
                if(!marketBook.Instruments[leg.MarketID].IsMarketGood)
                {
                    IsAllLegsGood = false;
                    return IsAllLegsGood;
                }
            }
            return IsAllLegsGood;
        }
        ...
        // *****************************************************
        /// <summary>
        /// Called by a model who would like to try and get upcoming economic data for its pricing legs during start up.
        /// The query will be then handled and data should be available when the model recieves the MarketInstrumentInitialized
        ...
        protected void RequestEconomicData(DateTime startLocal, DateTime endLocal)
        {
            ...
            double rangeHours = Math.Round(round * (endUTC.Subtract(startUTC)).TotalHours) / round;
            // Log request.
            Log.BeginEntry(LogLevel.Minor, "PricingEngine.RequestEconomicData: {0}:{1} request economic data:", ParentStrategy.Name, m_EngineName);
            foreach (PriceLeg pricingLeg in m_Legs)
                Log.AppendEntry(" {0}", pricingLeg.InstrumentName);
            Log.EndEntry();
            // Request data
        ...
        public override void AddSubElement(IStringifiable subElement)
        {
            ...
            {
                PriceLeg priceLeg = (PriceLeg)subElement;
                if (priceLeg.IsPriceEngineLeg)          // If users have other uses for PriceLegs by flagging this false, we will ignore them.
                    m_Legs.Add(priceLeg);
            }
        }
        //
