...
namespace UV.Strategies.StrategyEngines.StopBookEngines
{
    ...
    public class StopBookEngine : Engine, IStringifiable
    {
        ...
        private double m_StopBaseMultiplier = 1.0;	// Externally controlled multiplier for stop.



        // Stop tables
        protected List<double> m_StopPriceList = new List<double>();
        protected Dictionary<double, int> m_StopPriceQty = new Dictionary<double, int>();  // this is a bit scary to use as double as a key, but as long as we are careful we may get away with it
        private bool m_IsStopTriggered = false;
        protected int m_StopSign = 0;                    // zero means no activated stops
        ...
        // Blocking
        protected bool m_IsBlockingTimeOut = true;		// whether we allow unblocking because of a timeout.
        protected int m_BlockingTimeOut = 120;			// 120 periods of MA updates (usually 1 second each) for blocking to be in effect.
        protected int m_BlockingTimeOutRemaining = 0;
        private bool m_IsBlockingResetOnCrossing = true;	// unblocking caused by FairValue cross over.


        // non saved work space.
        protected List<double> m_PriceToDelete = new List<double>();	// reused workspace!
        protected List<double> m_PriceToAdd = new List<double>();	    // reused workspace!
        ...
        public double StopBaseMultiplier
        {
            get { return m_StopBaseMultiplier; }
            set
            {
                m_StopBaseMultiplier = value;
        ...
        //
        /// <summary>
        /// Called when a user would like to stop out of a position for various reasons.
        /// For example if an event is about to occur, (close of trading day, economic event, etc)
        /// the user can simply call this function to alert the stop book. a custom timeout is allowed 
        /// for additional control.
        /// </summary>
        /// <returns></returns>
        /// <param name="stopReason"></param>
        /// <param name="timeOut"></param>
        public bool CreateNewStopViolation(string stopReason, int timeOut)
        {
            if (m_LastPosition == 0) { return false; }			// exit immediately
            if (m_IsStopTriggered) { return false; }			// we are already working to exit a stop condition.
            int posSign = Math.Sign(m_LastPosition);
            ...
            // Create stopping condition now.
            m_IsStopTriggered = true;
            m_StopSign = Math.Sign(m_LastPosition);	// If the short-side stops out, then StopSign < 0.

            if (m_IsBlockingTimeOut)
                m_BlockingTimeOutRemaining = timeOut; // set blocking timeout.

            if (IsLogWrite)
            {
                Log.NewEntry(LogLevel.Major, "StopBookEngine: {0} custom stop for qty={1} reason={2} currentpos={3}."
               , m_Name, m_StopPriceQty[stopPrice].ToString(), stopReason, m_LastPosition.ToString());
            }
        ...
        /// <returns></returns>
        /// <param name="stopReason"></param>
        public bool CreateNewStopViolation(string stopReason)
        {
            return CreateNewStopViolation(stopReason, m_BlockingTimeOut);
        }
        //
        //
        // ****					Is Stop Triggered()				****
        ...
        //
        //
        protected virtual void UpdateTrailingStops(double mktPrice)
        {
            int posSign = Math.Sign(m_LastPosition);
            ...
            for (int i = 0; i < m_StopPriceList.Count; ++i)				// loop thru each stop-price in list.
            {
                ...
                double S = (m_StopBase * m_StopBaseMultiplier);
        ...
        //
        //
        protected void BroadcastParameters()
        {
        ...
        string IStringifiable.GetAttributes()
        {
            ...
            s.AppendFormat("StopBase={0}", m_StopBase);
            s.AppendFormat(" Multiplier1={0}", this.StopBaseMultiplier);
            if (this.IsBlockingTimeOutAllowed)
                s.AppendFormat(" BlockingTimeout={0}", this.BlockingTimeOut);
            s.AppendFormat(" IsBlockingResetOnCrossing={0}", this.IsBlockingResetOnCrossing);
            return s.ToString();
        }
            ...
            foreach (KeyValuePair<string, string> keyValPair in attributes)
            {
                ...
                switch (keyValPair.Key)
                {
                    case "StopBase":
                        ...
                        break;
                    case "Multiplier1":
                        this.StopBaseMultiplier = Convert.ToDouble(valStr);
                        break;
