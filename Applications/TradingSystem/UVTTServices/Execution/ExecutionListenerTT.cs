...
namespace UV.TTServices.Execution
{
    ...
    /// </summary>
    public class ExecutionListenerTT : ExecutionListener
    {
        #region Members
        // *****************************************************************
        // ****                     Members                             ****
        ...
        // ****                     Constructors                        ****
        // *****************************************************************
        public ExecutionListenerTT(string name, IEngineHub engineHub)
            : base(name, engineHub)
        {
        ...
        //******************************************************************
        /// <summary>
        /// Note: Currently we don't care about last price traded or volume, so for speed don't update thosee
        /// Additionally, we only use "direct" qty and prices, no implieds
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="eventArgs"></param>
        private void PriceSubscription_Updated(object sender, FieldsUpdatedEventArgs eventArgs)
        {
            ...
            // Note: Theoretically since this is memory is only accessed by a single thread and we call the event for our 
            // subscribers to look at, we can reset these variables at the end of our method here.  For now, lets leave it 
            // as if for safety sake, but it would result in a slight speed up during market updates
            market.BestDepthUpdated[MarketBase.BidSide] = market.DeepestLevelKnown + 1;   // reset our updates to base state
            ...
            market.IsLastTickBestPriceChange = false;
            ...
            for (int changedDepth = 0; changedDepth < market.DeepestLevelKnown; changedDepth++)
            {
                ...
                if (changedFieldIds.Length > 0)
                {
                    ...
                    { // if it is a snapshot, everything gets updated anyways
                        isBidPriceChange = changedFieldIds.Contains<FieldId>(FieldId.DirectBidPrice);
                        if (!isBidPriceChange) // only check if it isn't a price update on this side
                            isBidQtyChange = changedFieldIds.Contains<FieldId>(FieldId.DirectBidQuantity);
                        isAskPriceChange = changedFieldIds.Contains<FieldId>(FieldId.DirectAskPrice);
                        if (!isAskPriceChange)
                            isAskQtyChange = changedFieldIds.Contains<FieldId>(FieldId.DirectAskQuantity);
                    }
                    // *****************************************************
                    // ****             MarketBase updates              ****
                    // *****************************************************
                    ...
                    if (changedDepth == 0)
                    {
                        ...
                        if (isFireMarketChangeEvent)
                        {
                            ...
                            {
                                market.IsLastTickBestPriceChange = true;
        ...
        private void ProcessInstrumentsFound(UVInstrName uvInstrName, InstrumentDetails ttInstrDetails)
        {
            ...
            base.ProcessInstrumentsFound(uvInstrDetails);
        }
        //
        ...
        private void TT_OrderFilled(object sender, TradingTechnologies.TTAPI.OrderFilledEventArgs eventArgs)
        {
            ...
            { // we found our order id
                ...
                { // we found the uv instrument 
                    ...
                    if (!m_ExecContainer.m_OrderInstruments[uvInstr].TryProcessFill(fillEvent))                     // this will process the order state changes and send out appropiate events
                        Log.NewEntry(LogLevel.Error, "TT_OrderFilled : TT Order {0} Filled : UV Order ID {1} : TT_OrderFilled failed to find live order to process",
                                    eventArgs.NewOrder.SiteOrderKey,     // 0    
                                    uvOrderId);
                    if (isCompleteFill)
                ...
                {
                    Log.NewEntry(LogLevel.Error, "TT_OrderFilled: Matching UV Order Instrument Not Found for {0}", siteKey);
                }
            }
            else
            {
                Log.NewEntry(LogLevel.Error, "TT_OrderFilled: Matching TT Order Not Found for {0}", siteKey);
            }
        }
        //
        // *************************************************************
            ...
            { // we found our order id, this is an order from this execution container
                ...
                { // we found the uv instrumnet 
                    ...
                    { // since we have the order, lets just update everything now.
                        Log.BeginEntry(LogLevel.Minor, "TT Order {4} Added : {0} {1} {2} @ {3}",
                                    uvInstr,                            // 0
                        ...
                        if (uvOrderToUpdate.OrderType == UVOrderType.StopLimitOrder)
                        { // if this is a stop order take care of the extra fields here.
                            Log.AppendEntry(" - Stop Price {0}", eventArgs.Order.StopPrice);
                            uvOrderToUpdate.IStopPriceConfirmed = ttOrder.StopPrice.ToTicks() / ttOrder.InstrumentDetails.SmallestTickIncrement;
                        }
                        Log.EndEntry();

                        int mktSign = UV.Lib.Utilities.QTMath.MktSideToMktSign(uvOrderToUpdate.Side);
                        uvOrderToUpdate.OriginalQtyConfirmed = ttOrder.OrderQuantity * mktSign;  // UV uses signed QTY's
                        uvOrderToUpdate.ExecutedQty = ttOrder.FillQuantity * mktSign;
                        ...
                        { // we have pending modifications to this order, currently we place priority on price rather than qty, 
                            ...
                            // TODO: Add methods for stop limit orders
                        }
                    }
                    else
                    {
                ...
                {
                    Log.NewEntry(LogLevel.Error, "TT_OrderAdded: Matching UV Order Instrument Not Found for {0} - {1}", siteKey);
                }
            }
            else
            {
                Log.NewEntry(LogLevel.Error, "TT_OrderAdded: Matching TT Order Not Found for {0}", siteKey);
            }
        }
        //
        //
        // *************************************************************
        // ****             TT_OrderBookDownloaded                  ****
        // *************************************************************
        /// <summary>
        /// Process TT Order book download on start up
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="eventArgs"></param>
        private void TT_OrderBookDownloaded(object sender, OrderBookDownloadEventArgs eventArgs)
        {
            Log.NewEntry(LogLevel.Major, "TT_OrderBookDownloaded: Found {0} order's to manage on startup.", eventArgs.Orders.Count);
            foreach(Order ttOrder in eventArgs.Orders)
            {
                int existingUVOrderID;
                if (m_MapTT2UV.TryGetValue(ttOrder.SiteOrderKey, out existingUVOrderID))    // make sure we don't already somehow know about this order
                    continue;                                                               // Note TryGetValue is fastser than ContainsKey.

                UVInstrName uvInstr;
                if (m_TTInstrKeyToUVInstr.TryGetValue(ttOrder.InstrumentKey, out uvInstr))
                {   // if this is an instrument we are trading we should have the key matched
                    UVOrder uvOrder;
                    if(TTConvertNew.TryConvert(ttOrder, out uvOrder))
                    {   // we succesffuly created a UV order, we now need to add it to the default book.
                        uvOrder.Instrument = uvInstr;                                                           // assign our instrument to our order.
                        if(m_ExecContainer.m_OrderInstruments[uvInstr].TryAddToDefaultBook(uvOrder))
                        {
                            Log.NewEntry(LogLevel.Major, "TT_OrderBookDownloaded: Succesfully mapped TT Order {0} to UV Order {1}. Saved in default book", ttOrder.SiteOrderKey, uvOrder.Id);
                            m_MapTT2UV[ttOrder.SiteOrderKey] = uvOrder.Id;                                      // create all needed mappings
                            m_MapUV2TT[uvOrder.Id] = ttOrder.SiteOrderKey;
                            m_TTOrders[ttOrder.SiteOrderKey] = ttOrder;
                            m_ExecContainer.m_OrderInstruments[uvInstr].ProcessFoundOrder(uvOrder);             // this will call appropiate events, and set pending changes flag
                        }
                    }
                }
                Log.NewEntry(LogLevel.Error, "TT_OrderBookDownloaded: Failed to find UV Instrument for TT Instrument {0}. Order will be discarded", ttOrder.InstrumentDetails.Name);
            }
        }
        //
        // *************************************************************
        ...
        private void TT_OrderRejected(object sender, OrderRejectedEventArgs eventArgs)
        {
            ...
            { // replace was rejected, previous order still exists
                Log.NewEntry(LogLevel.Error, "TT Replace Order {0} Rejected : {1}",
            ...
            { // TODO : add to pending and figure out how to recall it.
                Log.NewEntry(LogLevel.Error, "TT Delete Order request {0} Rejected : {1}",
                          eventArgs.Order.SiteOrderKey,   //0
                          eventArgs.Order.Message);       //1
            }
        }
            ...
            { // we found our order id
                ...
                { // we found the uv instrumnet 
                    ...
                    { // since we have the order, lets just update everything now.
                        ...
                        m_ExecContainer.m_OrderInstruments[uvInstr].ProcessUpdateConfirm(uvOrderToUpdate);      
                        // trigger event her for order updated
        ...
        /// 
        /// if provided account string is not empty, order will also be filtered by account 
        /// </summary>
        /// <param name="instrKey"></param>
        /// <param name="account"></param>
        private void SubscribeToOrdersForInstrument(InstrumentKey instrKey, string account)
        {
            ...
            ts.OrderBookDownload += new EventHandler<OrderBookDownloadEventArgs>(TT_OrderBookDownloaded);

            // now add desired filters...
            TradeSubscriptionAndFilter filters = new TradeSubscriptionAndFilter(false, "filtercollection");  // this is a colleciton of all our desired filters 
            TradeSubscriptionInstrumentFilter tradeSubscriptionInstrumentFilter = new TradeSubscriptionInstrumentFilter(m_TTService.session, instrKey, false, "InstrFilter");
            filters.AddFilter(tradeSubscriptionInstrumentFilter);

            TradeSubscriptionAccountFilter tradeSubscriptionAccountFilter;
            if (account != string.Empty && account != null)
            { // if we are given an accunt to filer on, add it to the list.
                if (account.Length > 15)
                    account = account.Substring(0, 15); // can only be 15 chars!
                tradeSubscriptionAccountFilter = new TradeSubscriptionAccountFilter(account, false, string.Format("AccountFilter={0}", account));
                filters.AddFilter(tradeSubscriptionAccountFilter);
            }

            ts.SetFilter(filters);
            ts.Start();
        }
        //
        //
        ...
        protected override void ProcessCreateBook(UVOrderBook orderBook)
        {
            ...
            {   // We do not have an order instrument for this Instrument.
                    ...
                    m_InstrumentNameToTTKey.TryGetValue(orderBook.Instrument, out ttKey))
                {
                    ...
                    SubscribeToOrdersForInstrument(ttKey, orderBook.Account);
                }
            }
            else
            {
        ...
        private void ProcessSubmitOrder(UVOrder order, InstrumentKey instrKey)
        {
            ...
            profile.AccountType = AccountType.P2;
            profile.AccountName = order.Account;
            profile.OrderTag = order.OrderReason.ToString();
            ...
            if (order.OrderType == UVOrderType.StopLimitOrder) // if this is a stop order type, place the stop price.
                profile.StopPrice = Price.FromDouble(instrument, order.StopPricePending);
        ...
        private void ProcessDeleteOrder(UVOrder order)
        {
            ...
            { // this mean somehow this order never got correctly submitted to TT.  Since this order is only internal we can deal with it now.
                ...
                Log.NewEntry(LogLevel.Error, "Could not find correspsonding TT Order for UV Order Id with TT Site Key - Delting UV Order Permanently", order, ttSiteOrderKey);
            }
        }
        //
        //
        ...
        private void ProcessChangeOrderPrice(UVOrder order)
        {
            ...
            { // we found a matching tt site key
                ...
                { // we found the order from that site key
                    ...
                    if (order.IPriceConfirmed != order.IPricePending || order.OriginalQtyConfirmed != order.OriginalQtyPending)
                    {
                        ...
                        if (profile.Session.SendOrder(profile))
                        {
                            Log.NewEntry(LogLevel.Minor, "TT Order {0} was modified successfully UV Order : {1}", ttSiteOrderKey, order);
                            order.ChangesPending = true;        // flag order as waiting for confirmation
                        }
                        else
                        {
                    ...
                    Log.NewEntry(LogLevel.Error, "ProcessChangeOrderPrice: {0} could not be modified, no matching TT order found.", order);
            }
            else
                Log.NewEntry(LogLevel.Error, "ProcessChangeOrderPrice: {0} could not be modified, no matching TT Site Key found.", order);
        }
        //
        //
        // *************************************************************
            ...
            { // we found a matching tt site key
                ...
                { // we found the order from that site key
                    ...
                    if (order.OriginalQtyConfirmed != order.OriginalQtyPending)
                    {
                        ...
                        if (profile.Session.SendOrder(profile))
                        {
                            Log.NewEntry(LogLevel.Minor, "TT Order {0} Qty was modified successfully UV Order : {1}", ttSiteOrderKey, order);
                            order.ChangesPending = true;        // flag order as waiting for confirmation
                        }
                        else
                        {
                    ...
                    Log.NewEntry(LogLevel.Error, "ProcessChangeOrderQty: {0} could not be modified, no matching TT order found.", order);
            }
            else
                Log.NewEntry(LogLevel.Error, "ProcessChangeOrderQty: {0} could not be modified, no matching TT Site Key found.", order);
        }
        //
        //
        // ************************************************
        ...
        // *****************************************
        /// <summary>
        /// Called by user to submit an order to the TT market.
        /// </summary>
        /// <param name="orderBookID"></param>
        /// <param name="order"></param>
        /// <returns></returns>
        public override bool TrySubmitOrder(int orderBookID, UVOrder order)
        {
        ...
        // *****************************************
        /// <summary>
        /// called by user to delete an order, this will set pending state
        /// to dead and pending qty to 0
        /// /// </summary>
        /// <param name="order"></param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        public override bool TryChangeOrderPrice(UVOrder order, int newIPrice, int newQty)
        {
            if (base.TryChangeOrderPrice(order, newIPrice, newQty))
            {
        ...
        public override bool TryChangeOrderQty(UVOrder order, int newQty)
        {
            if (base.TryChangeOrderQty(order, newQty))
            {
        ...
        private void ProcessEvent()
        {
            ...
            while (m_WorkQueue.Count > 0)
            {
                ...
                else if (e is OrderEngineEventArgs)
                {   // execution side messaging between execution listeners!
                    m_ExecContainer.IOrderEngine.ProcessEvent(e);
                }
        ...
        public override void TimerSubscriberUpdate()
        {
            ...
            m_Dispatcher.BeginInvoke(CallITimerSubscribers);
        }
        //
