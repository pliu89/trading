...
namespace UV.TTServices.Fills
{
    ...
    public class FillHub : Hub , IStringifiable , IService
    {
        ...
        private ConcurrentDictionary<InstrumentKey, BookLifo> m_FillBooks = new ConcurrentDictionary<InstrumentKey, BookLifo>();        
        private ConcurrentDictionary<InstrumentName, InstrumentMapEntry> m_BookNameMap = new ConcurrentDictionary<InstrumentName, InstrumentMapEntry>(); // used by outsiders.        
        private ConcurrentDictionary<InstrumentKey, InstrumentMapEntry> m_BookKeyMap = new ConcurrentDictionary<InstrumentKey, InstrumentMapEntry>();

        // Private objects for fills waiting for books, etc.
        private Dictionary<InstrumentKey, SortedList<DateTime, FillEventArgs>> m_FillsWaitList = new Dictionary<InstrumentKey, SortedList<DateTime, FillEventArgs>>();
        private Dictionary<InstrumentKey, SortedList<DateTime, FillEventArgs>> m_NewFillsList = new Dictionary<InstrumentKey, SortedList<DateTime, FillEventArgs>>();
        private List<InstrumentName> m_InstrumentsWithoutBooks = new List<InstrumentName>();                // instruments for which we will NOT keep books (or they will be deleted later).
        private bool m_IsInitializingBooks = false;
        ...
        public FillHub(string name, bool isLogViewVisible)                                      // name is used to distinguish between multiple fillhubs.
            : base( name , UV.Lib.Application.AppInfo.GetInstance().LogPath, isLogViewVisible, LogLevel.ShowAllMessages)
        {
        ...
        {          
            if (m_TTService.IsRunning)  // TODO: We should check whether ServiceState is connected, then we can connect.
                this.HubEventEnqueue(new UV.Lib.OrderHubs.OrderHubRequest(UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestConnect));                  
        }
        ...
        // ****                 Request()                       ****
        //
        public void Request(UV.Lib.OrderHubs.OrderHubRequest request)
        {
        ...
        // ****             Try Enter Read Book()               ****
        // 
        public bool TryEnterReadBook(InstrumentName instrument, out UV.Lib.OrderHubs.FillBookLifo positionBook)
        {
            positionBook = null;
            InstrumentMapEntry mapEntry;
            if (m_BookNameMap.TryGetValue(instrument, out mapEntry))                    // If this mapping exists, the other entries will exist.
            {                                                                       // Note: always create new entries in this dictionary LAST. 
                BookLifo book;
                if (m_FillBooks.TryGetValue(mapEntry.Key, out book))
                {
                    ...
                    return false;                                                   // Failed to find book for instrument.
            }
            else
            {
        ...
        {
            InstrumentMapEntry mapEntry;
            if (m_BookNameMap.TryGetValue(instrument, out mapEntry))
            {
                BookLifo book;
                if (m_FillBooks.TryGetValue(mapEntry.Key,out book))
        ...
        {
            this.HubEventEnqueue(new UV.Lib.OrderHubs.OrderHubRequest(UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestShutdown));
        }//Stop()
        ...
        protected override void HubEventHandler(EventArgs[] eventArgList)
        {
            ...
            foreach (EventArgs eventArg in eventArgList)
            {
                ...
                try
                {
                    ...
                    else if (eventType == typeof(UV.Lib.MarketHubs.FoundServiceEventArg))
                        ProcessFoundMarketServiceResource((UV.Lib.MarketHubs.FoundServiceEventArg)eventArg);    // MarketHub found resources.
                    else if (eventType == typeof(UV.Lib.MarketHubs.MarketStatusChangedEventArg))
                        ProcessMarketStatusChanged((UV.Lib.MarketHubs.MarketStatusChangedEventArg)eventArg);          // Market status change.
                    else if (eventType == typeof(UV.Lib.OrderHubs.OrderHubRequest))
                        ProcessRequest((UV.Lib.OrderHubs.OrderHubRequest)eventArg);
                ...
                {
                    this.Log.Flush();
                    System.Windows.Forms.DialogResult result = UV.Lib.Application.ExceptionCatcher.QueryUserTakeAction(ex,this,eventArg);
                }
            }
        }//HubEventHandler()
        //
        ...
        /// Called by this hub thread.
        /// </summary>
        private void ProcessFill(FillEventArgs eventArg)
        {
        ...
        private void ProcessRejectedFill(RejectedFills.RejectedFillEventArgs eventArg)
        {
        ...
        /// </summary>
        private void ProcessFoundMarketServiceResource(UV.Lib.MarketHubs.FoundServiceEventArg eventArg)
        {
            ...
            {   // New instruments found.  We are guaranteed by Mkt hub that the InstrumentName is unique.
                ...
                foreach (UV.Lib.Products.InstrumentName instr in eventArg.FoundInstruments)
                {
                    ...
                    {   // Mkt has provided all mkt details for this instrument.
                        ...
                        {   // The above conditions should be trivial, since we add entries into the maps and FillBook list to satisfy the above.
                            ...
                            {   // Everything is perfectly self-consistent
                                ...
                                if (book.m_DollarPerPoint != dollarPerPoint)
                                {
                                    Log.AppendEntry(" DollarPerPt {0} -> {1}", book.m_DollarPerPoint, dollarPerPoint);
                                    book.m_DollarPerPoint = dollarPerPoint;
                                }
                                if (book.m_SmallestFillPriceIncr != minTickSize)
                                {
                                    Log.AppendEntry(" SmallestFillPriceIncr {0} -> {1}", book.m_SmallestFillPriceIncr, minTickSize);
                                    book.m_SmallestFillPriceIncr = minTickSize;
                                }
                            ...
                            {   // Inconsistent mappings.  
                                ...
                                {   
                                    ...
                                    if (book.NetPosition != 0)
                                    {
                                        ...
                                        // Add new book.
                                        if (m_FillBooks.TryAdd(newKey, book))
                                        {
                                            book.Name = newName;
        ...
        /// </summary>
        /// <param name="eventArg"></param>
        private void ProcessMarketStatusChanged(UV.Lib.MarketHubs.MarketStatusChangedEventArg eventArg)
        {
        ...
        /// </summary>
        private void ProcessRequest(UV.Lib.OrderHubs.OrderHubRequest eventArg)
        {
            if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestDropCopyNow)
            {
                if ( m_DropRules!=null)
                    m_DropRules.StartNewDropArchive();
            }
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestDropCopyArchive)
            {
                if (m_DropRules!=null)
                    m_DropRules.StartNewDropArchive();                                      // I archive drop copies on every fillbook drop now.
            }
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestConnect)
            {   // Connecting is first thing we do after TTAPI user is logged in (session exists) and hub thread is started.                
                ...
                if (m_Listener == null)                                                     // Used to detect whether we've already connected.
                {
                    ...
                    //if (!string.IsNullOrEmpty(m_UserAccount) )// either of these means "listen to all fills regardless of accounts"
                    //    m_Listener.FilterByAccount(m_UserAccount);                          // this fill hub carries fills from only ONE account ID.
            ...
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestShutdown)
            {
                if (m_DropRules != null) 
                    m_DropRules.Stop();
            ...
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestCompleteBookReset)
            {
            ...
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestRealPnLReset)
            {
                m_DropRules.Enqueue(this);
                ResetAllFillBooks(eventArg.Data);           // optionally, user can request RealPnLs to be set to values in Data[], or this can be a null.
                m_DropRules.Enqueue(this);
            }
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestCreateBook)
            {
            ...
            else if (eventArg.Request == UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestDeleteBook)
            {
        ...
        private void ResetAllFillBooks(object[] data, bool eraseAllBooks=false)
        {
            ...
            if (data != null && data.Length > 0)    // if data is null/empty, user wants to do a usual "daily reset all book"
            {
                ...
                while (ptr < data.Length)
                {
                    ...
                    //
                    InstrumentMapEntry mapEntry;
                    if (isInstrumentProvided && m_BookNameMap.TryGetValue(instrumentName,out mapEntry))
                    {
                        InstrumentKey key = mapEntry.Key;
                        m_FillBooks[key].Lock.EnterWriteLock();
                        if (double.IsNaN(realPnL) && double.IsNaN(realStartPnL))
                            m_FillBooks[key].ResetRealizedDollarGains();
                        else
                            m_FillBooks[key].ResetRealizedDollarGains(realPnL, realStartPnL);
            ...
            {   // User wants to completely erase the contents of all books.
                foreach (InstrumentKey key in m_FillBooks.Keys)
                {
                    ...
                    if (qty != 0)
                    {
                        ...
                        // Add fills
                        UV.Lib.OrderHubs.Fill fill = UV.Lib.OrderHubs.Fill.Create(-qty, price, DateTime.Now, DateTime.Now);
        ...
        protected override void UpdatePeriodic()
        {
            ...
            {   // Lets reset the PnL if we passed the NextResetDateTime.
                ...
                if (m_DropRules != null)    // need drop rules to "reset pnl" - since it saves position.
                {
                    ...
                    this.Request(new UV.Lib.OrderHubs.OrderHubRequest(UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestRealPnLReset));
                }
            }
            // Check to drop
        ...
        private void AddFillToBook(BookLifo lifoBook, FillEventArgs eventArg)
        {
            // Add fill to book          
            ...
            if (isFillAccepted)
            {
                if (eventArg.Type != FillType.InitialPosition)                  // Recall "initial position" are those already loaded from a drop file.
        ...
        // ****                 AddToNewFillsList()                     ****
        // *****************************************************************
        private void AddToNewFillsList(FillEventArgs eventArg)
        {
            ...
            {   // This is the first fill we have gotten for this key, need to request new book.
                ...
                UV.Lib.OrderHubs.OrderHubRequest requestEvent = new UV.Lib.OrderHubs.OrderHubRequest(UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestCreateBook);
        ...
        private bool IsInstrumentDesired(InstrumentName instrumentName)
        {
            UV.Lib.Products.ProductTypes type = instrumentName.Product.Type;

            bool isNotGood = (type == UV.Lib.Products.ProductTypes.Spread)
                    || (type == UV.Lib.Products.ProductTypes.Unknown)
                    || (type == UV.Lib.Products.ProductTypes.AutoSpreaderSpread)
                    || (type == UV.Lib.Products.ProductTypes.Strategy);
        ...
        private void LoadSavedBooks()
        {
            ...
            {
                foreach (UV.Lib.OrderHubs.FillBookLifo book in m_FillBooks.Values)
                {
                    book.RecalculateAll();
            ...
            else if (m_DropRules.TryLoadBooks( string.Format("FillBooks_{0}.txt",this.Name)) )// Try to load the old-name for the drop file!  
            {   // This provide smooth upgrading to new drop file name format.
                foreach (UV.Lib.OrderHubs.FillBookLifo book in m_FillBooks.Values)
                {
                    book.RecalculateAll();
                    PositionBookChangedEventArgs eventArgs = new PositionBookChangedEventArgs();
            ...
            /*
            string dropDirPath = string.Format("{0}{1:yyyy-MMM-dd}\\", UV.Lib.Application.AppInfo.GetInstance().DropPath,DateTime.Now);
        ...
        /// 
        /// </summary>
        private void ValidateInitialFills()
        {
            ...
            foreach (InstrumentKey instrumentKey in keyList)
            {
                ...
                if (m_FillsWaitList.TryGetValue(instrumentKey, out fillList) && fillList.Count > 0)
                {
                    ...
                    {   // We don't have a book for this instrument.  Request the creation of new books.
                        ...
                        UV.Lib.OrderHubs.OrderHubRequest hubRequest = new UV.Lib.OrderHubs.OrderHubRequest(UV.Lib.OrderHubs.OrderHubRequest.RequestType.RequestCreateBook);
        ...
        public List<IStringifiable> GetElementsDrop()
        {
            ...
            foreach (InstrumentName instrumentName in m_BookNameMap.Keys)
            {
                ...
                UV.TTServices.Fills.BookLifo fillBook;               
        ...
        void IStringifiable.SetAttributes(Dictionary<string, string> attributes)
        {
            ...
            foreach (string key in attributes.Keys)
                if (key.Equals("LocalTime") && DateTime.TryParse(attributes[key], out dt))
                    ...
                    this.NextResetDateTime = dt;//this.m_DropRules.NextResetDateTime = dt;
        }
        void IStringifiable.AddSubElement(IStringifiable subElement)
        {
            ...
            if (elementType == typeof(BookLifo))// if (subElement is BookLifo)
            {
                ...
                {
                    if ( m_FillBooks.TryAdd(mapEntry.Key, book) )
                        Log.NewEntry(LogLevel.Minor, "AddSubElement: FillBook {0}.", book);
                    else
                        Log.NewEntry(LogLevel.Error, "AddSubElement: FillBook {0} wasn't loaded. Another book with same key is present.", book);
                    }
                else
                {
        ...
        //
        private void OnPositionBookCreated(PositionBookChangedEventArgs eventArgs)
        {
        ...
        //
        private void OnPositionBookDeleted(PositionBookChangedEventArgs eventArgs)
        {
        ...
        //
        private void OnPositionBookChanged(PositionBookChangedEventArgs eventArgs)
        {
        ...
        //
        private void OnFillRejectionsUpdated(RejectedFills.RejectedFillEventArgs eventArgs)
        {
