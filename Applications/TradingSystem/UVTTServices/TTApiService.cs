...
namespace UV.TTServices
{
    ...
    public class TTApiService : IDisposable, IStringifiable, IService
    {
        ...
        public void Start()
        {
            ...
            // Get a copy of the Alert service
            m_AppServices = UV.Lib.Application.AppServices.GetInstance();
            IService service = null;
            if (m_AppServices.TryGetService("AlertManager", out service))
            {
                m_AlertManager = (AlertManager)service;
            }            
        ...
        /// </summary>
        private void TT_UniversalInitComplete(TTAPI api, Exception ex)
        {
            if (ex == null)
            {
                m_UAPI = (UniversalLoginTTAPI)api;                                // API instance we created asynchronously.
            ...
            else
            {
                ...
                if (m_AlertManager != null)
                    m_AlertManager.RequestEmailAlert(AlertLevel.High, "UniversalInitComplete: API universal initialization failed: {0}", ex.Message);
            }
        ...
        private void TT_AuthenticationStatusUpdate(object sender, AuthenticationStatusUpdateEventArgs eventArg)
        {
            ...
            if (m_AlertManager != null)
                m_AlertManager.RequestEmailAlert(AlertLevel.High, "AuthenticationStatusUpdate: Login authentication failed: ", eventArg.Status.StatusMessage);
