...
namespace UV.TTServices
{
    ...
    public static class TTConvertNew
    {
            ...
            if (TryConvert(ttInstr.Product, out UVProduct))                          // Determine the product type first. 
            {
                ...
                {   // Found exchange name.
                    ...
                    {   // The product name doesn't seem to be embedded in the 
                        ...
                        if (niceName.Contains("ICS"))
                        {// we are dealing with an ICS lets try and change the name a bit 
                            string cleanString = niceName.Remove(niceName.IndexOf("ICS"), "ICS: ".Length);
                            string regExpSearch = "\\d+x";
                            string ratio = "";
                            foreach (System.Text.RegularExpressions.Match match in System.Text.RegularExpressions.Regex.Matches(cleanString, regExpSearch))
                                ratio = ratio + match.Value;
                            ratio = ratio.Remove(ratio.Length - 1);
                            ratio = System.Text.RegularExpressions.Regex.Replace(ratio, ".{1}", "$0 ");
                            ratio = ratio.Remove(ratio.Length - 1);

                            regExpSearch = @" [a-zA-Z]{3}[0-9]{2}";
                            string monthCodes = "";
                            foreach (System.Text.RegularExpressions.Match match in System.Text.RegularExpressions.Regex.Matches(cleanString, regExpSearch))
                                if (monthCodes == "")
                                    monthCodes = match.Value;

                            niceName = string.Format("ICS: {0} - {1}", ratio, monthCodes);
        ...
        /// a new order each time.  All pending states will equal confirmed states
        /// </summary>
        ...
        public static bool TryConvert(Order ttOrder, out UVOrder uvOrder)
        {
            ...
            try
            {
                ...
                uvOrder.IPricePending = uvOrder.IPriceConfirmed;
                uvOrder.TickSize = ToUVTickSize(ttOrder.InstrumentDetails);
                uvOrder.OrderStateConfirmed = ToUVOrderState(ttOrder.TradeState);
                uvOrder.OrderStatePending = uvOrder.OrderStateConfirmed;
                uvOrder.Account = ttOrder.AccountName;
                Enum.TryParse<UV.Lib.OrderBooks.OrderReason>(ttOrder.OrderTag, out  uvOrder.OrderReason);   // attemp to parse reason to our enum
            }
            catch (Exception)
            {
                isSuccess = false;
